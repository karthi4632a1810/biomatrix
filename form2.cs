...
namespace Main_Biometric_upload
{
    ...
    string device_name, string status)
        {
            this.Invoke((MethodInvoker)delegate {
                this.dataGridView1.DataSource = null; this.dataGridView1.Rows.Clear(); this.dataGridView1.Refresh(); SqlDataAdapter adapt; con.Open(); DataTable dt = new DataTable();
        ...
        private void getattloop()
        {
            try
            {
                ...
                string t_machine = ""; string enroll_no = ""; string e_machine = ""; string v_mode = ""; string punch_dt = ""; string device_id = ""; string ip_address = ""; string tmmm = ""; string hhmm = "";
        ...
        t_machine, enroll_no, e_machine, v_mode, punch_dt, device_id, ip_address, tmmm, hhmm, m_id);
            }
            catch { }
        }
    ...
    string t_machine, string enroll_no, string e_machine, string v_mode, string punch_dt, string device_id, string ip_address, string tmmm, string hhmm, string m_id)
        {
            try
            {
                this.Invoke((MethodInvoker)delegate { label6.Text = "0"; }); string hhh = (label10.Text + "/webapi.php?p_id=" + p_id + "&t_machine=" + t_machine + "&enroll_no=" + enroll_no + "&e_machine=" + e_machine + "&v_mode=" + v_mode + "&punch_dt=" + punch_dt + "&device_id=" + device_id + "&ip_address=" + ip_address + "&tmmm=" + tmmm + "&hhmm=" + hhmm);
